...
if uploaded_file is not None and run_analysis:
    ...
    with st.spinner(
        ...
        high_risk_pairs = pd.DataFrame()
        pair_info = []

        if distance_df.empty:
            st.warning(
                "Not enough objects detected to calculate distances. Please upload an image containing at least two detectable objects."
            )
        else:
        ...
        with right_col:
            ...
            if high_risk_pairs.empty:

                st.info(
                    "No object pairs available."
                )
